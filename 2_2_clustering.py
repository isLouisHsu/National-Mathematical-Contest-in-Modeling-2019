...
plt.grid()
plt.savefig("images/2_2_pca.png")
plt.show()
...
scores = []
n_clusters_list = list(range(3, 20))
for n_clusters in n_clusters_list:
    pipline.set_params(gmm__n_components=n_clusters)
    pipline.fit(features)
    scores += [pipline.score(features)]

plt.figure()
plt.title("GMM' Scores(n_components=%d)" % n_components)
plt.plot(n_clusters_list, scores)
plt.xlabel("Number of Clusters")
plt.ylabel("Score")
plt.grid()
plt.savefig("images/2_2_gmm.png")
plt.show()
...
np.savetxt("output/2_2_seqLabels_pca%d_gmm%d.txt" % (n_components, n_clusters), y.astype(np.int))
joblib.dump(pipline, "output/2_2_model_pca%d_gmm%d.pkl" % (n_components, n_clusters))

# -------------------------------------------------------------------
cluster_centers_ = pipline.named_steps.gmm.means_
...
np.savetxt("output/2_2_pca_explained_variance_.txt", pipline.named_steps.pca.explained_variance_)
np.savetxt("output/2_2_pca__components_.txt", components_)
np.savetxt("output/2_2_pca__mean_.txt", mean_)
np.savetxt("output/2_2_gmm__cluster_centers_.txt", cluster_centers_)
np.savetxt("output/2_2_gmm__cluster_centers_inv.txt", clusterCenters)
